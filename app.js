    ...
    client.on('ready', () => {
        ...
        inquirer.prompt([
            ...
            makedir(`./${dir[0]}`).then(() => {
                ...
                var emotes = client.guilds.get(result[1]).emojis.map(a =>{return a.url})
                succesful = 0
                failed = 0
                for(i = 0;emotes.length > i;i++) {
                    emoteName = emotes[i].split('/').splice(-1)
                    request.get(emotes[i]).on('error', (err) => {console.log(err); failed++}).pipe(fs.createWriteStream(`./${dir[0]}/${emoteName}`))
                    succesful++
                }
                console.log(`${chalk.green(`${succesful} emotes succesfully retrieved!`)}\n ${chalk.red(`${failed} failed.`)}\n\nIf you would like to steal more emotes please relaunch`)
            })
        })
    })                          
...
function clear() {
    ...
    console.log(chalk.red(figlet.textSync('Emote_Stealer', { horizontalLayout: 'full' })))
}
